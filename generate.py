...
from autoCG.utils import arranger

from autoCG.Calculator import gaussian
from autoCG.Calculator import orca
...
class GuessGenerator:
    ...
    def find_spectator_molecules(self,reactant,reaction_info):
        ...
        reactant_atom_indices_for_each_molecule = reactant.get_atom_indices_for_each_molecule()
        molecule_indices = []
        ...
        not_used_molecule_indices = list(set(range(len(reactant_atom_indices_for_each_molecule))) - set(molecule_indices))
        spectating_molecules = [molecule_list[i] for i in not_used_molecule_indices]

        used_atom_indices = []
        for i in molecule_indices:
            used_atom_indices = used_atom_indices + reactant_atom_indices_for_each_molecule[i]
    ...
    def get_oriented_RPs(self,reactant,product,reaction_info=None,chg=None,multiplicity=None,save_directory=None):
        ...
        if reaction_info is None:
            ...
            if reaction_info is None:
                self.write_log('Internal error occured for finding reaction information! Guess generation terminated ...')
                exit()
        else:
        ...
        reaction_info, mapping_info, spectating_molecules = self.find_spectator_molecules(reactant,reaction_info)
        if len(spectating_molecules) == 0:
            ...
            if product is not None:
                reduced_product = process.get_reduced_intermediate(product,mapping_info)
            else:
                reduced_product = None
        ...
        self.write_log(f'[{et}] {len(ts_molecules)} ts conformers generated... Taken time: {et-st}\n\n')
        ...
        for bond in bond_list:
            if bond in reaction_info['f']:
                ts_constraints[bond] = self.make_constraint(molecule,bond,self.ts_scale)
        ...
        for i in range(n):
            ...
            name = f'guess_{i+1}'
            os.system(f'mkdir -p {folder_directory}')
            self.write_log(f'Start geometry optimization for {i+1}th guess of TS structure.\n')
            # Log changed into the specific folder directory
            self.save_directory = folder_directory
            self.write_log(f'{i+1}th Initial guess generation\n\nInitial\n{self.get_geometry(ts_molecules[i])}',file_name=name,mode='w')
        ...
        for i in range(n):
            ...
            # Back to original ...
            self.save_directory = save_directory           
            self.write_log(f'[{datetime.datetime.now()}] {i+1}th reactant-product pair well created !!!\n')
            ...
            matching_result[0],coeff = process.is_same_connectivity(reduced_reactant,reactant_molecules)
            #print (reduced_reactant.get_smiles('ace'))
            if reduced_product is not None:
                #print ('Checking P connectivity ...')
                matching_result[1],coeff = process.is_same_connectivity(reduced_product,product_molecules)
        ...
        self.write_log(f'[{endtime}] Total {endtime-starttime} was taken for making {n} RP pairs.\n')
        self.save_directory = None

        return reactant_product_pairs,reaction_info,mapping_info,matching_results # Reduced indices for reaction_info


    def get_oriented_RPs_from_smiles(self,reaction_smiles,chg=None,multiplicity=None,save_directory=None):
        ...
        return self.get_oriented_RPs(reactant,product,None,chg=chg,multiplicity=multiplicity,save_directory=save_directory)


    def get_oriented_RPs_from_geometry(self,input_directory,chg=None,multiplicity=None,save_directory=None):
        ...
        return self.get_oriented_RPs(reactant,None,reaction_info,chg=chg,multiplicity=multiplicity,save_directory=save_directory)
...
if __name__ == '__main__':        
    ...
    if '>>' in args.input:
        ...
        RP_pairs,reaction_info,mapping_info,matching_results = generator.get_oriented_RPs_from_geometry(args.input,save_directory=save_directory)
